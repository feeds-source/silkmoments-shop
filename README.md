# Silk Moments

Femme silk atelier shop (TanStack Start PWA) and the Shopify Online Store 2.0 theme **1.3.0**.

## Shopify theme

Ready-to-upload theme lives in [`shopify-theme/`](./shopify-theme) and as its own repo:

**[github.com/feeds-source/femme-silk-atelier](https://github.com/feeds-source/femme-silk-atelier)** (`main`)

Zip (theme folders at zip root — no wrapper, no `.git`):
- [`public/Femme-Silk-Atelier.zip`](./public/Femme-Silk-Atelier.zip)
- [`public/femme-shopify-theme.zip`](./public/femme-shopify-theme.zip) (same file)

Shopify: Online Store → Themes → Upload zip file. Do not unzip first.

## App (this repo)

```
npm install
npm run dev
```

Preview shop, bag, sizes, size guide, inventory. Cloudflare Worker shop remains on [vite-react-template](https://github.com/feeds-source/vite-react-template).
